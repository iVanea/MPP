...
public class EmployeeBST {
	...
	public EmployeeBST(NameComparator nc) {
		this.nc = nc;
		root = null;
	}
	...
	private boolean find(Employee x, Node n) {
		...
		if (n != null && n.element.equals(x))
			return true;
		return (nc.compare(x, n.element) < 0) ? find(x, n.left) : find(x, n.right);
	...
	public void insert(Employee x) {
		if (root == null) {
			...
			while (!inserted) {
				...
				else if (nc.compare(x, n.element) >= 0) {
	...
	public static void main(String[] args) {
		...
		for (int i = 3; i >= 0; --i) {
			bst.insert(new Employee("Person "+i, 10506*i, 2008+i, 1+i,1));
			bst.insert(new Employee("Person "+(i+15), 9515*i, 2000+i, 1+i,1));
		}
		bst.printTree();
//		bst.printAsDiagram();
		int i = 1;
		System.out.println("Is EMPLOYEE Person "+ i +" in the tree? " +
		bst.find(new Employee("Person "+i, 10506*i, 2008+i, 1+i,1)));
