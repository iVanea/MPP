...
public class Problem1 {
	...
	public static List<String> elementsInJustOne(List<String> list1, List<String> list2) {
		//implement
//		System.out.println(Stream.concat(list1.stream().filter(x->!list2.contains(x)), list2.stream().filter(x->!list1.contains(x))).collect(Collectors.toList()));
		return Stream.concat(list1.stream().filter(x->!list2.contains(x)), list2.stream().filter(x->!list1.contains(x))).collect(Collectors.toList());
	}
	...
	public static List<String> getAllFairfieldCustomers(List<Customer> list) {
		//implement
//		System.out.println(list.stream().filter(x->x.getCity().equals("Fairfield")).map(x->x.getName())
//				.sorted().collect(Collectors.toList()));
		return list.stream().filter(x->x.getCity().equals("Fairfield")).map(x->x.getName())
				.sorted().collect(Collectors.toList());
	}
